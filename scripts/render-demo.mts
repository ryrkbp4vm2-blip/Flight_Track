...
function glyph(ac: Aircraft, selected: boolean): string {
  ...
  const color = selected ? COLOR_SELECTED : altitudeColor(altOf(ac));
  ...
  return open + `<path d="${GLYPH[cls]}" fill="${color}"${stroke}/></g>`;
}
...
function statusBar(): string {
  const y = H - 44;
  const segs = "14 military aircraft     updated 2s ago";
  return [
    panel(12, y, 520, 34),
    text(26, y + 22, "14", { size: 14, weight: "bold", color: "#5ad1ff" }),
    text(46, y + 22, "military aircraft", { size: 14 }),
    text(176, y + 22, "updated 2s ago", { size: 14, color: "#8b97a4" }),
    `<rect x="300" y="${y + 8}" width="92" height="18" rx="4" fill="#ff7a59"/>`,
    text(346, y + 21, "SAMPLE DATA", { size: 11, weight: "bold", color: "#1a0c06", anchor: "middle" }),
    `<rect x="404" y="${y + 6}" width="116" height="22" rx="8" fill="none" stroke="rgba(120,150,180,0.2)"/>`,
    text(462, y + 21, "Trails: Selected", { size: 12, color: "#8b97a4", anchor: "middle" }),
    void segs && "",
  ].join("");
}
...
function renderSelected(): string {
  const sample = buildSample();
  const selHex = "3fbe88"; // GAF642 (A332 heavy) over Germany
  const selected = sample.ac.find((a) => a.hex === selHex)!;
  const planes = sample.ac.map((ac) => glyph(ac, ac.hex === selHex)).join("");
  return (
    `<svg xmlns="http://www.w3.org/2000/svg" width="${W}" height="${H}" viewBox="0 0 ${W} ${H}">` +
    defs() +
    baseMap() +
